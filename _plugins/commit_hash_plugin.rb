...
Jekyll::Hooks.register :site, :after_init do |site|
  begin
    ...
    puts "Git commit hash loaded: #{commit_hash[0..7]}"
  rescue => e
    puts "Warning: Could not load git commit hash: #{e.message}"
    # Set fallback values
    site.data['git'] ||= {}
...
Jekyll::Hooks.register :site, :post_render do |site|
  site.pages.each do |page|
    if page.output_ext == '.html'
      ...
      unless page.output.include?('name="git-commit"')
        meta_tag = %Q(  <meta name="git-commit" content="#{commit_hash}">)
        
        # Insert after the first meta tag in head
        if page.output.match(/<meta[^>]*>/i)
          page.output = page.output.sub(/(<meta[^>]*>)/i) do |match|
            "#{match}\n#{meta_tag}"
          end
        end
      end
