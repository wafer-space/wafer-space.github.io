  ...
  (site.pages + site.documents).each do |doc|
    if doc.output_ext == '.html'
      ...
      unless doc.output.include?('name="git-commit"')
        ...
        # Insert after the first meta tag in head
...
# Hook to add commit hash comment to HTML documents
Jekyll::Hooks.register :documents, :post_render do |doc|
  if doc.output_ext == '.html'
    commit_hash = doc.site.config['git_commit_hash'] || 'unknown'
    short_hash = doc.site.config['git_short_hash'] || 'unknown'
    ...
    unless doc.output.include?('Generated from git commit:')
      if doc.output.match(/<\/html>/i)
        doc.output = doc.output.sub(/<\/html>/i, "#{comment}\n</html>")
      else
...
# Hook to add commit hash comment to HTML pages
Jekyll::Hooks.register :pages, :post_render do |page|
  if page.output_ext == '.html'
    commit_hash = page.site.config['git_commit_hash'] || 'unknown'
    short_hash = page.site.config['git_short_hash'] || 'unknown'
    
    # Add HTML comment with commit info
    comment = %Q(<!-- Generated from git commit: #{commit_hash} (#{short_hash}) -->)
    
    # Add comment at the end of the document, before closing </html> tag
    unless page.output.include?('Generated from git commit:')
      if page.output.match(/<\/html>/i)
        page.output = page.output.sub(/<\/html>/i, "#{comment}\n</html>")
      else
        # Fallback if no closing html tag found
